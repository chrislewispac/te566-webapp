...
export default function MainDashboard() {
  ...
  let total_assets =
    (parseFloat(total_current_assets) + parseFloat(total_fixed_assets)).toFixed(
      2
    ) || 0;
  ...
  let net_worth =
    (parseFloat(total_assets) - parseFloat(total_liabilities)).toFixed(2) || 0;

  let total_liabilities_and_net_worth =
    (parseFloat(net_worth) + parseFloat(total_liabilities)).toFixed(2) || 0;

  if (isEmpty(balance_sheet) || !isLoaded(balance_sheet)) {
